...
function publicBracketHtml(matches){
  ...
  const card=m=>`<div class="public-bracket-card"><div class="${m.winner_cp_id===m.red?.cp_id?'winner':''}"><span>${esc(m.red?.name||'BYE')}</span><b>${m.red_score}</b></div><div class="${m.winner_cp_id===m.blue?.cp_id?'winner':''}"><span>${esc(m.blue?.name||'BYE')}</span><b>${m.blue_score}</b></div>${m.reason?`<small>${esc(reasonText(m.reason))}</small>`:''}</div>`;
  ...
  return `<div class="public-mirror-bracket">${left}<div class="public-final-column"><h4>Финал</h4>${card(final)}${bronze?`<div class="public-third"><h4>За 3-е место</h4>${card(bronze)}</div>`:''}</div>${right}</div>`;
}
...
function renderPublicDashboard(data){
  ...
  const details=data.categories.map(c=>{const cat=c.category;let body='';const groupNames=Object.keys(c.group_tables||{});if(groupNames.length)body+=`<div class="public-group-tables">${groupNames.map(g=>publicGroupTableHtml(g,c.group_tables[g])).join('')}</div>`;if(['swiss','round_robin'].includes(cat.format)&&c.standings?.length)body+=`<div class="public-group-tables">${publicGroupTableHtml(cat.format==='swiss'?'Swiss':'Все со всеми',c.standings)}</div>`;body+=publicBracketHtml(c.matches||[]);return body?`<section class="public-category-panel"><div class="public-category-head"><h3>${esc(cat.name)}</h3><span>${esc(({knockout:'Олимпийская',groups:'Группы + плей-офф',swiss:'Швейцарская',round_robin:'Все со всеми'})[cat.format]||cat.format)}</span></div>${body}</section>`:''}).join('');

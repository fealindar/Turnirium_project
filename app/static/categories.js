...
function categoryListHtml(){
  const active=adminState.categories.filter(c=>c.status!=='completed'), archived=adminState.categories.filter(c=>c.status==='completed');
  const row=(c,archivedRow=false)=>`<button type="button" class="category-list-item ${c.id===adminState.selectedCategoryId?'active':''} ${archivedRow?'archived':''}" data-open-category="${c.id}"><span class="category-list-main"><b>${esc(c.name)}</b><small>${esc(categoryFormatName(c.format))}</small></span><span class="category-list-state">${archivedRow?'Архив':'Ред.'}</span></button>`;
...
function templateToolsHtml(prefix,locked=false){
  ...
  return `<div class="template-inline"><div class="field template-picker"><label>Заполнить правила из шаблона</label><select id="${prefix}TemplateSelect"><option value="">— выбрать шаблон —</option>${opts}</select></div><button type="button" id="${prefix}ApplyTemplate" ${locked?'disabled':''}>Применить</button><button type="button" id="${prefix}DeleteTemplate" class="danger ghost" disabled>Удалить</button></div><div class="template-save-inline"><div><b>Сохранить текущие правила как шаблон</b><small>Сохраняются формат, таймер, начисление очков и остальные параметры формы.</small></div><input id="${prefix}TemplateName" placeholder="Например: Сабля"><button type="button" id="${prefix}SaveTemplate">Сохранить шаблон</button></div>`;
}
...
function renderCategories(){
  ...
  const creating=!c;
  $('#adminContent').innerHTML=`<div class="category-workspace"><aside class="card category-sidebar">${categoryListHtml()}</aside><section class="category-main"><div class="card category-editor"><div class="category-editor-head"><div><span class="eyebrow">${creating?'Создание':'Редактирование'}</span><h2>${creating?'Новая категория':esc(c.name)}</h2><p>${creating?'Сначала задайте правила категории. После создания их можно менять до начала первого реального боя.':locked?(c.status==='completed'?'Категория завершена. Правила доступны только для просмотра.':'Первый бой уже начался: формат, очки и правила зафиксированы. Название можно изменить.'):'Формат, начисление очков и остальные правила можно менять до начала первого реального боя.'}</p></div>${c?`<span class="pill ${locked?'warn':'live-pill'}">${c.status==='completed'?'Архив':locked?'Правила зафиксированы':'Правила редактируются'}</span>`:''}</div>${creating?`${templateToolsHtml('create',false)}${categoryFormHtml(categoryDefaults(),'createCatForm',false,'Создать категорию')}`:`${c.status==='completed'?'<div class="archive-banner">Категория завершена и находится в архиве.</div>':''}${locked&&c.status!=='completed'?'<div class="archive-banner">После начала первого боя правила и формат менять нельзя. До этого момента можно даже пересобрать уже сформированную сетку.</div>':''}${templateToolsHtml('edit',locked)}${categoryFormHtml(c,'editCatForm',locked,locked?'Сохранить название':'Сохранить изменения')}<div class="row category-actions">${c.status==='completed'?'<button id="reopenCategory">Вернуть в активные</button>':'<button id="completeCategory">✓ Завершить категорию</button>'}<button id="exportCategoryPdf">PDF результатов</button><button id="exportCategoryJson">JSON</button></div>`}</div>${c?categoryParticipantsHtml(c,locked):''}</section></div>`;
  $$('[data-open-category]').forEach(b=>b.onclick=async()=>{adminState.selectedCategoryId=Number(b.dataset.openCategory);await loadSelectedCategory();renderCategories();});
  $('#newCategoryBtn').onclick=()=>{adminState.selectedCategoryId=null;adminState.categoryParticipants=[];adminState.matches=[];renderCategories();};
  if(creating){
    const form=$('#createCatForm');bindCategoryFormatVisibility(form);bindTemplateTools('create',form,categoryDefaults(),false);
    form.onsubmit=async e=>{e.preventDefault();const data=categoryFormData(form,categoryDefaults());try{const created=await api(`/api/tournaments/${adminState.tournamentId}/categories`,{method:'POST',body:JSON.stringify(data)});await refreshAdminData(false);adminState.selectedCategoryId=created.id;await loadSelectedCategory();renderCategories();toast('Категория создана и открыта для редактирования')}catch(err){toast(err.message,true)}};
    return;
  }
  const form=$('#editCatForm');bindCategoryFormatVisibility(form);bindTemplateTools('edit',form,c,locked);
  form.onsubmit=async e=>{e.preventDefault();const data=categoryFormData(form,c);const structuralChanged=data.format!==c.format||data.group_target_size!==c.group_target_size||data.group_qualifiers!==c.group_qualifiers||data.swiss_rounds!==c.swiss_rounds;if(!locked&&structuralChanged&&adminState.matches.length&&!confirm('Изменение структуры удалит ещё не начатую сетку/туры. Результатов боёв ещё нет, поэтому это безопасно. Продолжить?'))return;try{await api(`/api/categories/${c.id}`,{method:'PUT',body:JSON.stringify(data)});await refreshAdminData(false);await loadSelectedCategory();renderCategories();toast(locked?'Название категории сохранено':structuralChanged&&adminState.matches.length?'Настройки сохранены. Сформируйте сетку заново.':'Настройки категории сохранены')}catch(err){toast(err.message,true)}};
  if($('#completeCategory')) $('#completeCategory').onclick=async()=>{if(!confirm(`Завершить категорию «${c.name}»? Она исчезнет из активных и останется в архиве.`))return;try{const r=await api(`/api/categories/${c.id}/complete`,{method:'POST'});await refreshAdminData(false);adminState.selectedCategoryId=c.id;await loadSelectedCategory();renderCategories();if(r.export_error)toast(`Категория завершена, но экспорт не создан: ${r.export_error}`,true);else toast('Категория завершена. PDF и JSON подготовлены.')}catch(e){toast(e.message,true)}};
  if($('#reopenCategory')) $('#reopenCategory').onclick=async()=>{try{await api(`/api/categories/${c.id}/reopen`,{method:'POST'});await refreshAdminData(false);adminState.selectedCategoryId=c.id;await loadSelectedCategory();renderCategories();toast('Категория возвращена в активные')}catch(e){toast(e.message,true)}};
...
function renderBracketTab(){
  ...
  $('#adminContent').innerHTML=`<div class="bracket-toolbar"><div class="field"><label>Категория</label>${categorySelectHtml()}</div>${c?`${archived?'<span class="pill warn">Архив · только просмотр</span>':buildControls}<button id="standingsBtn">Таблица результатов</button><button id="bracketExportPdf">PDF</button><button id="bracketExportJson">JSON</button>`:''}<span class="toolbar-hint">${archived?'Завершенная категория доступна для просмотра и экспорта результатов.':'В группах и текущем Swiss-туре можно переставлять бойцов, пока их бой ещё не начался.'}</span></div><div id="bracketBody" class="top-gap"></div>`;
...
function renderBracketBody(c){
  if(!adminState.matches.length){$('#bracketBody').innerHTML='<div class="empty-state compact"><h2>Сетка ещё не сформирована</h2><p>Система автоматически разведёт участников одного клуба, затем по возможности одного города.</p></div>';return;}
  let html='';
...
function renderGroupsLayout(){
  ...
  return `<section class="card"><div class="section-head"><div><h2>Состав групп</h2><p>${playoffExists?'Состав групп зафиксирован после создания плей-офф.':anyMovable?'Бойцов можно переставлять внутри группы и между группами до начала их первого боя. Уже выступившие бойцы зафиксированы.':'Все бойцы уже провели первый бой — состав зафиксирован.'}</p></div><span class="pill">Разведение: клуб → город</span></div><div class="groups-grid">${names.map(g=>`<div class="group-box"><div class="group-title"><b>Группа ${esc(g)}</b><span>${groups[g].length} уч.</span></div><div class="group-members" data-group="${esc(g)}">${groups[g].map(cp=>{const editable=movable(cp);return `<div class="group-member ${editable?'drag':'locked'}" draggable="${editable}" data-group-cp="${cp.id}">${editable?'<span class="drag-handle">⋮⋮</span>':'<span class="drag-lock">●</span>'}<div class="grow"><b>${esc(cp.name)}</b><small>${esc(affiliation(cp)||'Без клуба / города')}</small></div></div>`}).join('')}</div></div>`).join('')}</div></section>`;
}
...
function renderSwissStage(ms){
  ...
  return `<section class="card top-gap"><div class="section-head"><div><h2>Швейцарская система</h2><p>В текущем туре пары можно переставлять и после старта тура, пока конкретный бой ещё не начался. Начатые и завершённые пары фиксируются.</p></div></div><div class="swiss-rounds">${rounds.map(r=>{const rows=ms.filter(m=>m.round_no===r).sort((a,b)=>a.match_no-b.match_no),currentRound=r===current;let idx=0;const anyEditable=currentRound&&rows.some(m=>!swissMatchLocked(m));return `<div class="swiss-round ${anyEditable?'editable':''}" data-swiss-round="${r}"><div class="swiss-round-head"><h3>Тур ${r}</h3><span>${rows.every(m=>m.status==='finished')?'Завершён':anyEditable?'Можно менять не начатые пары':'Идёт'}</span></div><div class="swiss-pairs-grid">${rows.map(m=>{const a=idx++,b=idx++,locked=!currentRound||swissMatchLocked(m);const slot=(f,slotIdx)=>`<div class="swiss-fighter ${f?'':'bye-card'} ${locked?'locked':''}" data-swiss-slot="${slotIdx}" data-swiss-cp="${f?.cp_id||''}" data-swiss-locked="${locked?'1':'0'}" ${!locked&&f?'draggable="true"':''}><span class="drag-handle">${!locked?'⋮⋮':'●'}</span><span><b>${esc(f?.name||'BYE')}</b><small>${esc(affiliation(f)||'Свободный слот')}</small></span></div>`;return `<div class="swiss-pair">${slot(m.red,a)}<span class="vs">—</span>${slot(m.blue,b)}<small class="swiss-state">${m.red_score}:${m.blue_score} · ${esc(statusText(m.status))}${m.reason?' · '+esc(reasonText(m.reason)):''}${m.status==='finished'&&m.reason!=='BYE'?` <button class="link-edit" data-correct-match="${m.id}">Исправить</button>`:''}</small></div>`}).join('')}</div></div>`}).join('')}</div></section>`;
}

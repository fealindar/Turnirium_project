...
function categoryParticipantsHtml(c,locked){
  ...
  return `<div class="card category-roster"><div class="category-roster-head"><div><h2>Участники категории</h2><p>${locked?'Состав и посев зафиксированы. Исключение из категории недоступно после первого боя.':'Можно менять до первого боя.'}</p></div><span class="pill">${adminState.categoryParticipants.length} в категории</span></div><div class="category-roster-columns"><section class="roster-pane"><div class="roster-pane-head"><b>Состав / посев</b><span>${adminState.categoryParticipants.length}</span></div><div id="seedList" class="roster-scroll stack">${adminState.categoryParticipants.map(cp=>`<div class="list-item drag category-roster-participant ${cp.participant_status==='withdrawn'?'row-muted':''}" draggable="${!locked}" data-cp="${cp.id}"><span class="drag-handle">${locked?'●':'⋮⋮'}</span><div class="category-roster-person grow"><b>${esc(cp.name)}</b><span class="muted">${esc(affiliation(cp)||'Без клуба / города')}</span></div>${cp.participant_status==='withdrawn'?'<span class="pill warn">Выбыл</span>':''}${cp.disqualified?'<span class="pill warn">DSQ</span>':''}<div class="category-roster-actions"><button type="button" class="small ${cp.participant_status==='withdrawn'?'':'warning-btn'}" data-category-withdraw="${cp.participant_id}" data-next-status="${cp.participant_status==='withdrawn'?'active':'withdrawn'}">${cp.participant_status==='withdrawn'?'Вернуть':'Выбыл'}</button><button type="button" class="small danger ghost" data-unenroll="${cp.participant_id}" ${locked?'disabled title="Недоступно после начала боёв"':''}>Исключить</button></div></div>`).join('')||'<div class="roster-empty">Участников пока нет.</div>'}</div></section><section class="roster-pane"><div class="roster-pane-head"><b>Добавить участника</b><input id="categoryParticipantSearch" class="search compact-search" placeholder="Поиск…"></div><div id="availableParticipantsList" class="roster-scroll stack">${available.map(p=>categoryAvailableParticipantHtml(p,locked)).join('')||'<div class="roster-empty">Нет доступных участников.</div>'}</div></section></div></div>`;
}
...
function bindCategoryParticipantControls(c,locked){
  ...
  $$('[data-enroll]').forEach(b=>b.onclick=async()=>{if(locked)return;try{await api(`/api/categories/${c.id}/participants`,{method:'POST',body:JSON.stringify({participant_id:Number(b.dataset.enroll)})});adminState.categoryParticipants=await api(`/api/categories/${c.id}/participants`);adminUiState.participantCategories.clear();renderCategories();toast('Участник добавлен в категорию')}catch(e){toast(e.message,true)}});
  $$('[data-category-withdraw]').forEach(b=>b.onclick=async()=>{const participantId=Number(b.dataset.categoryWithdraw),next=b.dataset.nextStatus;const message=next==='withdrawn'?'Отметить участника как «Выбыл»? Этот статус действует на весь турнир и закроет его будущие сформированные бои.':'Вернуть участника в активные? Уже завершённые из-за выбытия бои автоматически не отменятся.';if(!confirm(message))return;try{await api(`/api/participants/${participantId}/status`,{method:'POST',body:JSON.stringify({status:next})});adminUiState.participantCategories.clear();await refreshAdminData(true);renderCategories();toast(next==='withdrawn'?'Участник отмечен как выбывший':'Участник возвращён')}catch(e){toast(e.message,true)}});
  $$('[data-unenroll]').forEach(b=>b.onclick=async()=>{if(locked)return;const participantId=Number(b.dataset.unenroll),cp=adminState.categoryParticipants.find(item=>item.participant_id===participantId);const reset=adminState.matches.length?' Сформированная сетка/туры будут сброшены и их потребуется сформировать заново.':'';if(!confirm(`Исключить ${cp?.name||'участника'} только из категории «${c.name}»?${reset}`))return;try{const result=await api(`/api/categories/${c.id}/participants/${participantId}`,{method:'DELETE'});adminUiState.participantCategories.delete(participantId);await refreshAdminData(true);renderCategories();toast(result.bracket_reset?'Участник исключён. Сформируйте сетку заново.':'Участник исключён из категории')}catch(e){toast(e.message,true)}});
}
